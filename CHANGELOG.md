...
- **AntigravityAdapter switches to `agy -p --output-format json`** — captures agy's top-level `thinking_tokens` / `cache_read_tokens` (previously discarded as plain text), fail-closed on a non-`SUCCESS` envelope status (agy exits 0 on failures). New gated live smoke (`test_smoke_antigravity.py`) is the envelope's falsifiability gate.
